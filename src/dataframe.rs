...
/// A plain enumeration of the possible data types used in a `DataFrame`.
...
pub enum Column {
    /// A Column consisting of Ints
    Int(Vec<i64>),
    /// A Column consisting of Bools
    Bool(Vec<bool>),
    /// A Column consisting of Floats
    Float(Vec<f64>),
    /// A Column consisting of Strings
    String(Vec<String>),
}
...
fn read_chunk<T>(schema: Vec<DataType>, reader: &mut T, from: u64, len: u64) -> Vec<Column>
where
    T: BufRead + Seek,
{
...
mod tests {
    ...
    fn test_read_file() {
        ...
        let parsed1: Vec<Column> = read_chunk(schema.clone(), &mut input, 0, 26);
        assert_eq!(parsed1, expected.clone());

        // last line is discarded
        let mut larger_input = Cursor::new(b"<1><1>\n<a><0>\n<1.2><>\n<no><1>");
        let parsed2: Vec<Column> = read_chunk(schema.clone(), &mut larger_input, 0, 27);
        assert_eq!(parsed2, expected.clone());

        // first line is discarded
        let mut input_skipped_l1 = Cursor::new(b"<b><1>\n<1><1>\n<a><0>\n<1.2><>");
        let parsed3: Vec<Column> = read_chunk(schema.clone(), &mut input_skipped_l1, 3, 26);
        ...
        let parsed4: Vec<Column> = read_chunk(schema.clone(), &mut input_with_invalid, 0, 32);
